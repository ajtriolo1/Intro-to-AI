...
class Agent:
    ...
    def proceed(self):
        ...
        mines = 0
        for i in range(len(self.mine_matrix)):
            for j in range(len(self.mine_matrix)):
    ...
    def reasoning(self):
        ...
        has_new_info = self.evaluate_equations(var_list, matrix) or self.evaluate_equations(var_list, rref_matrix)

        proceed = self.proceed()
        if has_new_info and proceed:
            # if new knowledge was produced, mark game map, update the knowledge base and regenerate/redo matrix/RREF.
            if len(self.kb) != 0:
                self.reasoning()
            else:
                self.random_guess()
    ...
    def random_guess(self):
        ...
        for i in range(len(self.mine_matrix)):
            for j in range(len(self.mine_matrix)):
                if self.mine_matrix[i][j] == -1:
                    self.update((i, j), random.randint(0, 1))
                    if len(self.kb) == 0:
                        self.random_guess()
    ...
    def update(self, coord: Tuple[int, int], val: {0, 1}):
        ...
        if self.mine_matrix[x][y] == -1:
            if self.env.mark(x, y, val):
                ...
                if val == 0:
                    ...
                    self.success += 1
            else:  # wrong reasoning occurred
                # print(f'Error: mark ({x}, {y}) as {val}')
                if val == 0:
                    self.mine_matrix[x][y] = 1
                else:
                    self.mine_matrix[x][y] = 0
